 ...
 *
 * Hem:    Undo/Redo | Styles | Font family + size + grow/shrink/clear |
 *         B I U S Sub Sup | Highlight + Color | Indent | Align | Lists | Line-spacing
 * Infoga: Table | Image | HR | Link
 * Layout: Line height | Paragraph spacing
 */
...
const HIGHLIGHT_COLORS = [
  { label: 'Gul',          color: '#ffff00' },
  { label: 'Ljusgrön',     color: '#00ff00' },
  { label: 'Turkos',       color: '#00ffff' },
  { label: 'Rosa',         color: '#ff00ff' },
  { label: 'Blå',          color: '#0000ff' },
  { label: 'Röd',          color: '#ff0000' },
  { label: 'Mörkblå',      color: '#00008b' },
  { label: 'Blågrön',      color: '#008080' },
  { label: 'Mörkgrön',     color: '#006400' },
  { label: 'Mörklila',     color: '#800080' },
  { label: 'Mörkröd',      color: '#8b0000' },
  { label: 'Mörkgul',      color: '#808000' },
  { label: 'Orange',       color: '#ffa500' },
  { label: 'Ljusgrå',      color: '#d3d3d3' },
  { label: 'Mörkgrå',      color: '#696969' },
];
...
export default function TopToolbar() {
  ...
  // One open dropdown at a time — identified by name
  ...
  const activeFontSize   = (editor.getAttributes('textStyle').fontSize   as string | undefined) ?? '13';
  ...
  function shrinkFont() {
    const cur = Number(activeFontSize);
    const prev = [...FONT_SIZES].reverse().find((s) => s < cur) ?? Math.max(8, cur - 2);
    editor!.chain().focus().setFontSize(String(prev)).run();
  }

  // ── Styles entries (shown rendered in their own style) ────────────────────

  const styleEntries = [
    { label: 'Normal',   ps: { fontSize: 13, color: '#1e1e1e', fontWeight: 400, fontFamily: 'Calibri, Arial, sans-serif' }, action: () => editor.chain().focus().setParagraph().run() },
    { label: 'Rubrik 1', ps: { fontSize: 20, color: '#1f3864', fontWeight: 700, fontFamily: 'Calibri, Arial, sans-serif' }, action: () => editor.chain().focus().toggleHeading({ level: 1 }).run() },
    { label: 'Rubrik 2', ps: { fontSize: 15, color: '#2e74b5', fontWeight: 700, fontFamily: 'Calibri, Arial, sans-serif' }, action: () => editor.chain().focus().toggleHeading({ level: 2 }).run() },
    { label: 'Rubrik 3', ps: { fontSize: 13, color: '#1f3864', fontWeight: 700, fontFamily: 'Calibri, Arial, sans-serif' }, action: () => editor.chain().focus().toggleHeading({ level: 3 }).run() },
  ];
  ...
  return (
    ...
    >
      ...
      {/* ── Ribbon content ─────────────────────────────────────────────── */}
      <div style={{ display: 'flex', alignItems: 'stretch', padding: '4px 6px', gap: 6, flexWrap: 'wrap', minHeight: 56 }}>
          ...
          <>
            ...
            <RibbonGroup label="Teckensnitt">
              ...
              <Dropdown
                ...
                trigger={
                  <span style={{ fontSize: 12, fontFamily: activeFontFamily + ', sans-serif', minWidth: 108, textAlign: 'left', color: '#1e1e1e' }}>
                    {activeFontFamily}
                  </span>
                }
                triggerTitle="Teckensnitt"
                minWidth={180}
              >
                {FONTS.map((f) => (
                  <DropdownItem
              ...
              <Dropdown
                ...
                trigger={
                  <span style={{ fontSize: 12, minWidth: 36, textAlign: 'center', color: '#1e1e1e', fontFamily: 'Calibri, Arial, sans-serif' }}>
                    {activeFontSize}
                  </span>
                }
                triggerTitle="Teckenstorlek"
                minWidth={64}
              >
                {FONT_SIZES.map((s) => (
                  <DropdownItem
                    key={s}
                    active={activeFontSize === String(s)}
                    onSelect={() => { editor.chain().focus().setFontSize(String(s)).run(); close(); }}
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (v && Number(v) > 0) editor.chain().focus().setFontSize(v).run();
                }}
                style={{
                  width: 40, height: 26, padding: '0 4px', fontSize: 12,
              ...
              {/* Grow / shrink */}
              <RBtn onClick={growFont}   title="Öka teckenstorlek (Ctrl+>)"><GrowIcon /></RBtn>
              <RBtn onClick={shrinkFont} title="Minska teckenstorlek (Ctrl+<)"><ShrinkIcon /></RBtn>
              ...
              <RBtn onClick={() => editor.chain().focus().toggleSubscript().run()}   active={editor.isActive('subscript')}   title="Nedsänkt (Ctrl+=)">
                <span style={{ fontSize: 12, fontFamily: 'Calibri, Arial, sans-serif', lineHeight: 1 }}>x<sub style={{ fontSize: 9 }}>2</sub></span>
              </RBtn>
              <RBtn onClick={() => editor.chain().focus().toggleSuperscript().run()} active={editor.isActive('superscript')} title="Upphöjt (Ctrl+Shift+=)">
                <span style={{ fontSize: 12, fontFamily: 'Calibri, Arial, sans-serif', lineHeight: 1 }}>x<sup style={{ fontSize: 9 }}>2</sup></span>
              </RBtn>
              ...
              <div style={{ position: 'relative' }}>
                <button
                  ...
                  title="Markera text"
                  style={colorBtnStyle(openMenu === 'highlight')}
                >
                  <HighlightIcon />
                ...
                {openMenu === 'highlight' && (
                  <ColorPalette
                    colors={HIGHLIGHT_COLORS.map((h) => h.color)}
              ...
              <div style={{ position: 'relative' }}>
                <button
                  ...
                  title="Teckenfärg"
                  style={colorBtnStyle(openMenu === 'color')}
                >
            ...
            <RibbonGroup label="Stycke">
              {/* Indent */}
              <RBtn onClick={() => editor.chain().focus().decreaseIndent().run()} title="Minska indrag (Shift+Tab)">
                <IndentDecIcon />
              </RBtn>
              <RBtn onClick={() => editor.chain().focus().increaseIndent().run()} title="Öka indrag (Tab)">
                <IndentIncIcon />
              </RBtn>
              ...
              <RBtn onClick={() => editor.chain().focus().setTextAlign('justify').run()} active={editor.isActive({ textAlign: 'justify' })} title="Justera"><AlignJustifyIcon /></RBtn>
              ...
              {/* Line spacing */}
              <div style={{ position: 'relative' }}>
                <RBtn onClick={() => toggle('linespacing')} active={openMenu === 'linespacing'} title="Radavstånd">
                  <LineSpacingIcon />
                </RBtn>
                {openMenu === 'linespacing' && (
                  <DropdownPanel minWidth={160}>
        ...
        {tab === 'infoga' && (
          <>
            ...
            <RibbonGroup label="Text">
              <BigBtn icon={<HrIcon big />} label="Avdelare" onClick={() => editor.chain().focus().setHorizontalRule().run()} />
            </RibbonGroup>
          </>
        )}
        ...
        {tab === 'layout' && (
          <>
            <RibbonGroup label="Radavstånd">
              ...
              <RBtn onClick={() => editor.chain().focus().unsetLineHeight().run()} title="Standardradavstånd">
                <ResetIcon />
              </RBtn>
            </RibbonGroup>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
      ...
      <p style={{
        ...
        fontFamily: 'Calibri, Arial, sans-serif',
        borderTop: '1px solid #e8e6e3', paddingTop: 2, marginTop: 'auto',
      }}>
        {label}
      </p>
    </div>
...
function GroupSep() {
  return <div style={{ width: 1, alignSelf: 'stretch', background: '#d2d0ce', margin: '0 2px' }} />;
}

function InlineSep() {
  return <div style={{ width: 1, height: 20, background: '#d2d0ce', margin: '0 1px', flexShrink: 0 }} />;
}

// Standard icon ribbon button
function RBtn({
...
// Large vertical button (Insert tab style)
  ...
  return (
    <button
      ...
      style={{
        ...
        gap: 2, padding: '4px 10px', minWidth: 50, height: 46,
        background: active ? '#ddeeff' : 'transparent',
        border: active ? '1px solid #c0d8f0' : '1px solid transparent',
        color: active ? '#004e8c' : '#1e1e1e',
    ...
    >
      {icon}
      <span style={{ fontSize: 10, fontFamily: 'Calibri, Arial, sans-serif', color: 'inherit' }}>{label}</span>
    </button>
  );
}
...
function DropdownPanel({ children, minWidth }: { children: React.ReactNode; minWidth?: number }) {
  return (
    <div style={{
      ...
      background: '#ffffff', border: '1px solid #d2d0ce',
...
function ColorPalette({ colors, active, onSelect, onClear, clearLabel }: {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 22px)', gap: 3, marginBottom: 6 }}>
        {colors.map((c) => (
          <button
            key={c}
            type="button"
...
function colorBtnStyle(active: boolean): React.CSSProperties {
  return {
    display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
...
function TableIcon({ big }: { big?: boolean })  { const s = big ? 20 : 13; return <svg width={s} height={s} viewBox="0 0 24 24" {...sv}><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="3" y1="15" x2="21" y2="15"/><line x1="12" y1="3" x2="12" y2="21"/></svg>; }
function HrIcon({ big }: { big?: boolean })     { const s = big ? 20 : 13; return <svg width={s} height={s} viewBox="0 0 24 24" {...sv}><line x1="3" y1="12" x2="21" y2="12"/><polyline points="7 8 3 12 7 16"/><polyline points="17 8 21 12 17 16"/></svg>; }
function ImageIcon({ big }: { big?: boolean })  { const s = big ? 20 : 13; return <svg width={s} height={s} viewBox="0 0 24 24" {...sv}><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>; }
function LinkIcon({ big }: { big?: boolean })   { const s = big ? 20 : 13; return <svg width={s} height={s} viewBox="0 0 24 24" {...sv}><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>; }
...
function ClearIcon()        { return <svg width="13" height="13" viewBox="0 0 24 24" {...sv}><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4z"/><line x1="2" y1="2" x2="22" y2="22" stroke="#c00000"/></svg>; }
