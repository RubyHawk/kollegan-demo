...
export function LatestOffersCard({
  ...
  return (
    <DashboardCard
      ...
      description={offers.length > 0 ? `${visibleOffers.length} senaste av ${total} offerter` : 'Dina senaste offerter visas här när de finns.'}
      action={(
      ...
      {offers.length === 0 ? (
        ...
        <>
            ...
            <table className="w-full min-w-[700px] text-left">
              ...
              <tbody className="divide-y divide-[var(--border-light)]">
                {visibleOffers.map((offer) => (
                  <tr key={offer.id} className="group transition-colors hover:bg-[var(--surface-1)]">
          ...
          <div className="divide-y divide-[var(--border-light)] md:hidden">
            {visibleOffers.map((offer) => (
              <Link key={offer.id} href={`/offerter/${offer.id}`} className="block px-4 py-3 transition-colors hover:bg-[var(--surface-1)]">
