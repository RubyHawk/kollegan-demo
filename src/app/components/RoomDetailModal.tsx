...
function AmenityBadge({ amenity }: { amenity: AmenityDef }) {
  return (
    <div className="flex items-center gap-1.5 bg-[var(--surface-alt)] border border-[var(--border)] rounded-lg px-2.5 py-1.5">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="text-amber-600 dark:text-amber-400 shrink-0">
...
/* Gradient per room type for the image area */
...
export default function RoomDetailModal({ room, onClose, onBooked }: Props) {
  ...
  const handleBook = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/rooms/book', {
        ...
        body: JSON.stringify({ room_id: room.id, guest_name: guestName.trim(), check_in: checkIn, check_out: checkOut }),
      });
      const data = await res.json();
      if (data.success) { onBooked(); } else { setError(data.message || 'Något gick fel.'); }
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) { onBooked(); } else { setError(data.message || 'Kunde inte avboka.'); }
    } catch { setError('Nätverksfel. Försök igen.'); }
    finally { setLoading(false); }
  };
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 dark:bg-black/70 backdrop-blur-sm dialog-overlay-enter p-4"
      onClick={onClose}
    >
      <div
        className="bg-[var(--surface)] rounded-2xl w-full max-w-2xl shadow-2xl border border-[var(--border)] room-detail-enter overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex flex-col md:flex-row min-h-[440px]">

          {/* ── Left: Room Image Area ── */}
          <div className={`relative md:w-[42%] min-h-[200px] bg-gradient-to-br ${TYPE_GRADIENT[room.type] ?? TYPE_GRADIENT.Enkel} flex flex-col items-center justify-center p-8 overflow-hidden`}>
            {/* Decorative pattern */}
            ...
            {/* Price badge */}
            <div className="relative z-10 mt-6 bg-white/15 border border-white/25 backdrop-blur-sm rounded-xl px-4 py-2 text-center">
              <div className="text-xl font-bold text-white">{meta.price.toLocaleString('sv-SE')} kr</div>
          ...
          <div className="flex-1 flex flex-col p-6 overflow-y-auto">
            ...
            <div className="flex items-start justify-between mb-4">
              ...
              <button
                onClick={onClose}
                className="hidden md:flex w-8 h-8 rounded-lg items-center justify-center text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--surface-alt)] transition-colors shrink-0 ml-2"
                aria-label="Stäng"
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" />
            ...
            {isBooked ? (
              <div className="space-y-4">
                <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-xl p-4 space-y-2.5">
                ...
                <div className="flex gap-3">
                  <button onClick={onClose} className="flex-1 text-sm text-[var(--text-secondary)] bg-[var(--surface-alt)] hover:bg-[var(--border)] rounded-xl py-3 font-medium transition-all active:scale-95">
                    Stäng
                  </button>
                  <button
              ...
              <form onSubmit={handleBook} className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1.5 uppercase tracking-wide">Gästnamn</label>
                  <input
                    type="text"
                    value={guestName}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1.5 uppercase tracking-wide">Incheckning</label>
                    <input
                      type="date"
                      value={checkIn}
                      onChange={(e) => setCheckIn(e.target.value)}
                      required
                      className="w-full bg-[var(--surface-alt)] border border-[var(--border)] rounded-xl px-3 py-3 text-sm text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 transition-all [color-scheme:light] dark:[color-scheme:dark]"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1.5 uppercase tracking-wide">Utcheckning</label>
                    <input
                      type="date"
                      value={checkOut}
                ...
                {/* Price summary */}
                <div className="bg-amber-50 dark:bg-amber-900/15 border border-amber-200 dark:border-amber-800/50 rounded-xl px-4 py-3 flex items-center justify-between">
                ...
                <div className="flex gap-3">
                  <button type="button" onClick={onClose} className="flex-1 text-sm text-[var(--text-secondary)] bg-[var(--surface-alt)] hover:bg-[var(--border)] rounded-xl py-3 font-medium transition-all active:scale-95">
                    Avbryt
                  </button>
                  <button
                    type="submit"
                    disabled={loading || !guestName.trim()}
                    className="flex-1 text-sm text-white bg-amber-600 hover:bg-amber-700 dark:bg-amber-500 dark:hover:bg-amber-400 dark:text-stone-900 rounded-xl py-3 font-semibold transition-all active:scale-95 disabled:opacity-40"
                  >
