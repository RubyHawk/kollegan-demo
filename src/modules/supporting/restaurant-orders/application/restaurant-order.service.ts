...
function normalizedItemsFromOrder(order: RestaurantOrderView): NormalizedOrderItem[] {
  return order.items.map((item) => ({
    ...
    modifierTotalCents: item.modifierTotalCents ?? 0,
...
export async function updateRestaurantOrder(
  ...
  } else if (input.discountCents !== undefined || input.taxRateBps !== undefined) {
    replacementTotals = calculateOrderTotals(normalizedItemsFromOrder(existing), existing.currency, {
      discountCents: input.discountCents ?? existing.discountCents ?? 0,
      taxRateBps: input.taxRateBps ?? existing.taxRateBps ?? 1200,
    });
  }
