...
export const handleCreateTemplate = createHandler(
  ...
  async (ctx) => {
    ...
    const template = await createTemplate(
      { organizationId: payload.orgId!, name: body.name, content: body.content },
      payload.sub,
    );
...
export const handleUpdateTemplate = createHandler(
  ...
  async (ctx) => {
    ...
    const id      = extractId(req);
    const payload = await requireStaff(req);
    const updated = await updateTemplate(id, payload.orgId!, { name: body.name, content: body.content });
