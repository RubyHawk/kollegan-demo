...
- Pull-request quality gates also keep the split `scripts/lib/plan-status*` helper modules on the same process-validation path as the main plan-status guard, so future maintenance of repo-backed completion reporting does not silently bypass CI classification.
