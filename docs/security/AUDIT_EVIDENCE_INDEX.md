...
| 2026-05-15 | Manual main redeploy trigger                               | Local working tree                                         | Added a `workflow_dispatch` path to the VPS deploy workflow so operators can redeploy the latest `main` release with the current artifact-free flow instead of rerunning an older historical push workflow that may still depend on retired artifact-upload steps               | Engineering lead |
