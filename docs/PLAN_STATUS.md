...
| --- | --- | ---: | --- |
| Engineering / refactor structure | Open | 0% | All 18 implementation-order items are structurally covered in the repo; inventory shows 1 legacy wrappers, 0 files above 1000 lines, 8 files above 500 lines, and 14 dead-candidate rows. |
...
| Tracked files scanned | 1272 |
| Source files scanned | 1031 |
| API route files | 157 |
| API v1 route files | 120 |
| Feature API clients | 31 |
| Legacy API compatibility wrappers | 1 |
| Files above 1000 lines | 0 |
| Files above 500 lines | 8 |
| Dead-candidate review rows | 14 |
...
| API v1 migration and wrappers | Open | Inventory shows 120 `/api/v1` route files, 31 feature API clients, and 1 legacy compatibility wrappers. |
| Cleanup and file-size enforcement | Open | Inventory shows 0 files above 1000 lines, 8 files above 500 lines, and 14 dead-candidate review rows. |
...
- The cleanup inventory currently reports `1` legacy API compatibility wrappers and `14` dead-candidate review rows.
- The inventory still reports `43` literal legacy `/api/*` references outside route files; the generated inventory distinguishes expected demo, public-document, OpenAPI, proxy, and integration rows from migration blockers.
